...
async function renderDashboardTasks() {
  ...
  try {
    const response = await fetch("/api/dashboard/tasks/top5");
...
// =============================================
// LEVEL-UP POPUP + CONFETTI BLAST
// =============================================
function initLevelUpPopup() {
  const levelElement = document.querySelector('[th\\:text="${dashboard.level}"]');
  ...
  if (currentLevel > previousLevel) {
    ...
    (function frame() {
      ...
      const opacity = Math.max(timeLeft / duration, 0);
      confetti({
        particleCount: 10,
        angle: 60,
        spread: 55,
    ...
    document.getElementById('levelUpModal').addEventListener('hidden.bs.modal', () => {
      confetti.reset();
    });
  }
